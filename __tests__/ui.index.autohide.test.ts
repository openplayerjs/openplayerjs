...
function makeAudioPlayer() {
  ...
  return new Player(a, { plugins: [] });
}

async function flushTimersAndMicrotasks() {
  // Don't flush *all* timers (that would execute future hide timers prematurely).
  // We only need to flush any 0ms timeouts and pending microtasks.
  jest.advanceTimersByTime(0);
  await Promise.resolve();
}

async function flushMicrotasks() {
  // allow queueMicrotask callbacks (used in maybeAutoplayUnmute)
  await Promise.resolve();
  await Promise.resolve();
}
...
describe('UI createUI + controls autohide', () => {
  ...
  test('autoplay-unmute button is removed when volumechange reports unmuted', async () => {
    ...
    const p = makeAudioPlayer();

    type AutoplayHack = Player & {
      canAutoplay: boolean;
