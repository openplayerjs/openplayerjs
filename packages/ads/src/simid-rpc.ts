 ...
 *
 * `SimidSession` extends this class and provides the higher-level protocol
 * logic (session init, creative ready, media bridging, lifecycle).
 */
...
export class SimidRpcChannel {
   ...
   */
  protected send(type: SimidPlayerMessage, args?: Record<string, unknown>): Promise<unknown> {
   ...
   * Returns true if the message was a resolve/reject that settled a pending promise.
   */
  protected resolvePending(data: SimidMessage): boolean {
    ...
    if (isReject) {
      ...
      if (p) {
        ...
        p.reject({ errorCode: data.errorCode ?? value?.errorCode, reason: data.reason ?? value?.message });
      }
      return true;
    }
  ...
  /** Remove the window message listener and reject any in-flight send() promises. */
