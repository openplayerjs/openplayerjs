 ...
 *
 * The low-level postMessage wire protocol (message counter, format detection,
 * origin pinning, send/resolve/reject, inbound parsing) lives in the
 * `SimidRpcChannel` base class; this file owns the higher-level session logic
 * (init handshake, creative-ready, media bridging, lifecycle).
 *
 * Message format notes:
 *  - Some SIMID creatives post messages as JSON-encoded strings rather than plain objects.
 *    SimidRpcChannel handles both and auto-detects the outgoing format.
...
import { SIMID_DEFAULT_HEIGHT_PX, SIMID_DEFAULT_WIDTH_PX } from './constants';
import { SIMID_CREATIVE, SIMID_MEDIA, SIMID_PLAYER } from './simid-protocol';
import type { SimidMessage } from './simid-protocol';
import { SimidRpcChannel } from './simid-rpc';

// Re-export the protocol constants so consumers importing from '@openplayerjs/ads'
// (via index.ts) keep working after the transport split.
export { SIMID_CREATIVE, SIMID_MEDIA, SIMID_PLAYER } from './simid-protocol';
...
export class SimidSession extends SimidRpcChannel {
  ...
  private handleMessage(data: SimidMessage) {
    // Let the RPC layer settle any pending resolve/reject first.
  ...
  private buildEnvironmentData(): Record<string, unknown> {
    ...
    const pw = Math.round(playerRect.width) || v.offsetWidth || SIMID_DEFAULT_WIDTH_PX;
    const ph = Math.round(playerRect.height) || v.offsetHeight || SIMID_DEFAULT_HEIGHT_PX;
