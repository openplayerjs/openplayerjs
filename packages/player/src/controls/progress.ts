...
export class ProgressControl extends BaseControl {
  ...
  protected build(): HTMLElement {
    ...
    const seekFromClientX = (clientX: number) => {
      ...
      const duration = getDuration();
      if ((core.isLive || duration === Infinity) && !this.activeOverlay) return;
      ...
      if ((val < currTime && !allowRewind) || (val > currTime && !allowSkip)) return;
    ...
    this.listen(
      ...
      (e: Event) => {
        ...
        const currTime = getValue();
        if ((val < currTime && !allowRewind) || (val > currTime && !allowSkip)) return;
        if (Number.isFinite(val)) core.currentTime = val;
