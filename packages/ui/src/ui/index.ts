...
export function createUI(player: Player, media: HTMLMediaElement, controls: Control[]) {
  ...
  labelElement(wrapper, player.config?.labels?.container || 'Media player');
  ...
  labelElement(mediaContainer, player.config?.labels?.media || 'Media');
  ...
  const scheduleHide = (ms?: number): void => {
    if (player.media.paused || player.media.ended) return;
    if (hideTimer) window.clearTimeout(hideTimer);
  ...
  const offPlaying = player.events.on('playing', () => scheduleHide(POINTER_SHOW_MS));
  const offPause = player.events.on('pause', () => showControls());
  const offEnded = player.events.on('ended', () => showControls());
