...
import { Player as CorePlayer } from '@openplayer/core';
...
export default class Player {
  ...
  async play() {
    if (!this.player) throw new Error('OpenPlayer.play() called before init()');
  ...
  load() {
    if (!this.player) throw new Error('OpenPlayer.load() called before init()');
    this.player.load();
  }
  ...
  addElement(el: HTMLElement, placement: ControlPlacement = { v: 'bottom', h: 'right' }) {
    (this.player as any).controls.addElement(el, placement);
  }

  addControl(control: Control) {
    if (!this.player) throw new Error('OpenPlayer.addControl() called before init()');
    (this.player as any).controls.addControl(control);
  }

  set src(source: string) {
    if (!this.player) throw new Error('OpenPlayer.src must be set after init()');
...
OpenPlayerAny.Core = Core;
try {
  Object.assign(OpenPlayerAny, Core);
} catch {
