...
describe('Player core', () => {
  ...
  test('play emits cmd:play immediately (user-gesture context) and does not double-fire; pause emits cmd:pause', async () => {
    ...
    // cmd:play fires synchronously (before any await) so that media.play() is
    // called while the user-gesture activation is still valid.
    expect(calls).toEqual(['play']);

    // Simulate the engine signaling readiness (resolves the play promise).
    p.events.emit('loadedmetadata');
    await playPromise;

    // cmd:play fired exactly once — no double-fire after readiness.
    expect(calls).toEqual(['play']);
